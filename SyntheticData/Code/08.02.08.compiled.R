...
for(J in ncol(Synthetic_Collapse):1)
{
  if(sum( Ind_synthetic_collapse[,J]) > 0)
  {
   Goal = Goal.tmp 
    ...
    data_bargunit_tmp = data.frame(data_bargunit,collapse=Collapse[,J])
    data_synthetic_bargunit_tmp = data.frame(data_synthetic_bargunit, collapse=Synthetic_Collapse[,J])
    
    
    ## Start Algorithm
    data_general=data_bargunit_tmp[Ind_collapse[,J] ,,drop=FALSE]
    data_synthetic_general=data_synthetic_bargunit_tmp[Ind_synthetic_collapse[,J],,drop=FALSE]
    variables=variables
    N.samples=1
