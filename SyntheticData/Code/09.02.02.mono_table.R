...
#  sapply(1:length(unique_mono_table),tmp_f3)
  ...
  if(length(unique_mono_table)>2000)
    Index = cbind(1+0:floor(length(unique_mono_table)/2000)*2000,c(1:floor(length(unique_mono_table)/2000)*2000,length(unique_mono_table)))
  
  if(length(unique_mono_table)<=2000)
    Index = cbind(1,length(unique_mono_table))
